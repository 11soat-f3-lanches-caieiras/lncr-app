...
public interface JpaCustomerRepository extends JpaRepository<JpaCustomerEntity, Integer> {
    ...
    @Query(value = "SELECT * FROM customer WHERE document_number = :documentNumber and email = :email", nativeQuery = true)
    Optional<JpaCustomerEntity> findByDocumentNumberAndEmail(@Param("documentNumber") String documentNumber, @Param("email") String email);
