...
import br.com.tp.lncr.app.datasources.postgres.fooditem.*;
import br.com.tp.lncr.app.datasources.storage.FoodItemImageStorageImpl;
...
class FoodItemDataProxyTest {
    ...
    void setUp() {
        ...
        foodItemImageDTO.set_data("base64data");
    ...
    void checkIfFoodItemExistsByNameReturnsTrue() {
        when(jpaFoodItemRepositoryImpl.existsByName(eq("X-Bacon"), any(), any())).thenReturn(true);

        boolean result = foodItemDataProxy.existsByName("X-Bacon");

        assertTrue(result);
        verify(jpaFoodItemRepositoryImpl).existsByName(eq("X-Bacon"), eq(jpaFoodItemRepository), eq(jpaFoodItemMapper));
    }

    @Test
    void checkIfFoodItemExistsByNameReturnsFalse() {
        when(jpaFoodItemRepositoryImpl.existsByName(eq("NonExistent"), any(), any())).thenReturn(false);

        boolean result = foodItemDataProxy.existsByName("NonExistent");

        assertFalse(result);
        verify(jpaFoodItemRepositoryImpl).existsByName(eq("NonExistent"), eq(jpaFoodItemRepository), eq(jpaFoodItemMapper));
    }
    ...
    void createFoodItemWithoutImages() {
        ...
        verify(jpaFoodItemRepositoryImpl).save(eq(foodItemDTO), eq(jpaFoodItemRepository), eq(jpaFoodItemMapper));
    ...
    void createFoodItemWithImages() {
        ...
        assertNotNull(result);
        assertEquals(foodItemDTO.getId(), result.getId());
        assertEquals("11.jpg", result.getImages().get(0).getFileName());
        verify(jpaFoodItemRepositoryImpl).save(eq(foodItemDTO), eq(jpaFoodItemRepository), eq(jpaFoodItemMapper));
        verify(jpaFoodItemImageRepositoryImpl).saveAll(any(), eq(jpaFoodItemImageRepository), eq(jpaFoodItemMapper));
    ...
    void findAllFoodItemsWithoutCategoryAndWithoutImages() {
        ...
        assertNull(result.get(0).getImages());
        verify(jpaFoodItemRepositoryImpl).getAllFoodItems(eq(10), eq(jpaFoodItemRepository), eq(jpaFoodItemMapper));
    ...
    void findAllFoodItemsWithCategoryAndWithImages() {
        ...
        assertNotNull(result);
        assertEquals(1, result.size());
        assertNotNull(result.get(0).getImages());
        assertEquals(1, result.get(0).getImages().size());
        verify(jpaFoodItemRepositoryImpl).getAllFoodItemsByCategory(eq(10), eq(1), eq(jpaFoodItemRepository), eq(jpaFoodItemMapper));
        verify(jpaFoodItemImageRepositoryImpl).findAllByFoodItemId(eq(1), eq(jpaFoodItemImageRepository), eq(jpaFoodItemMapper));
    }
    ...
    void findFoodItemByIdWithImages() {
        ...
        verify(jpaFoodItemRepositoryImpl).findById(eq(1), eq(jpaFoodItemRepository), eq(jpaFoodItemMapper));
        verify(jpaFoodItemImageRepositoryImpl).findAllByFoodItemId(eq(1), eq(jpaFoodItemImageRepository), eq(jpaFoodItemMapper));
    }
    ...
    void findFoodItemByIdReturnsNullWhenNotFound() {
        ...
        assertNull(result);
        verify(jpaFoodItemRepositoryImpl).findById(eq(999), eq(jpaFoodItemRepository), eq(jpaFoodItemMapper));
        verify(jpaFoodItemImageRepositoryImpl, never()).findAllByFoodItemId(any(), any(), any());
    }
    ...
    void findFoodItemByIdListReturnsMatchingItems() {
        ...
        assertNotNull(result);
        assertEquals(1, result.size());
        verify(jpaFoodItemRepositoryImpl).findByIdList(eq(ids), eq(jpaFoodItemRepository), eq(jpaFoodItemMapper));
    }
    ...
    void findFoodItemImageByIdReturnsImageWithData() throws IOException {
        ...
        verify(jpaFoodItemImageRepositoryImpl).findById(eq(1), eq(jpaFoodItemImageRepository), eq(jpaFoodItemMapper));
    ...
    void saveFoodItemImageSuccessfully() {
        ...
        assertNotNull(result);
        assertEquals(foodItemImageDTO.getId(), result.getId());
        verify(jpaFoodItemImageRepositoryImpl).save(eq(foodItemImageDTO), eq(jpaFoodItemImageRepository), eq(jpaFoodItemMapper));
    ...
    void deleteFoodItemWithImages() {
        ...
        verify(jpaFoodItemRepositoryImpl).deleteById(eq(1), eq(jpaFoodItemRepository));
        verify(jpaFoodItemImageRepositoryImpl).deleteByFoodItemId(eq(foodItemDTO.getImages()), eq(jpaFoodItemImageRepository), eq(jpaFoodItemMapper));
    ...
    void deleteFoodItemImage() {
        ...
        verify(jpaFoodItemImageRepositoryImpl).delete(eq(foodItemImageDTO), eq(jpaFoodItemImageRepository), eq(jpaFoodItemMapper));
