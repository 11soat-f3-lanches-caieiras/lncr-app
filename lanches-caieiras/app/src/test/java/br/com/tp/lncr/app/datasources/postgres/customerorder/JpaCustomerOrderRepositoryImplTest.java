...
class JpaCustomerOrderRepositoryImplTest {
    ...
    void shouldSaveCustomerOrderSuccessfully() {
        ...
        CustomerOrderDTO inputDto = createCustomerOrderDTO(null, 50.0, "PENDING", customer, LocalDateTime.now(), null);
        JpaCustomerOrderEntity mappedEntity = createJpaCustomerOrderEntity(null, 50.0, 1, 1, LocalDateTime.now(), null, "João Silva");
        JpaCustomerOrderEntity savedEntity = createJpaCustomerOrderEntity(1, 50.0, 1, 1, LocalDateTime.now(), null, "João Silva");
        CustomerOrderDTO expectedDto = createCustomerOrderDTO(1, 50.0, "PENDING", customer, LocalDateTime.now(), null);
    ...
    void shouldSaveCustomerOrderWithNullCustomer() {
        CustomerOrderDTO inputDto = createCustomerOrderDTO(null, 25.0, "PENDING", null, LocalDateTime.now(), null);
        JpaCustomerOrderEntity mappedEntity = createJpaCustomerOrderEntity(null, 25.0, 1, null, LocalDateTime.now(), null, null);
        JpaCustomerOrderEntity savedEntity = createJpaCustomerOrderEntity(1, 25.0, 1, null, LocalDateTime.now(), null, null);
        CustomerOrderDTO expectedDto = createCustomerOrderDTO(1, 25.0, "PENDING", null, LocalDateTime.now(), null);
    ...
    void shouldFindCustomerOrderByIdWhenExists() {
        Integer orderId = 1;
        CustomerOrderCustomerDTO customer = new CustomerOrderCustomerDTO(1, "João Silva");
        JpaCustomerOrderEntity entity = createJpaCustomerOrderEntity(1, 75.0, 2, 1, LocalDateTime.now(), null, "João Silva");
        CustomerOrderDTO expectedDto = createCustomerOrderDTO(1, 75.0, "PREPARING", customer, LocalDateTime.now(), null);
    ...
    void shouldFindCustomerOrdersByStatusList() {
        ...
        List<JpaCustomerOrderEntity> entities = Arrays.asList(
                createJpaCustomerOrderEntity(1, 50.0, 1, 1, LocalDateTime.now(), null, "João Silva"),
                createJpaCustomerOrderEntity(2, 75.0, 2, 2, LocalDateTime.now(), null, "Maria Santos")
        );

        CustomerOrderDTO dto1 = createCustomerOrderDTO(1, 50.0, "PENDING", customer1, LocalDateTime.now(), null);
        CustomerOrderDTO dto2 = createCustomerOrderDTO(2, 75.0, "PREPARING", customer2, LocalDateTime.now(), null);
    ...
    private CustomerOrderDTO createCustomerOrderDTO(Integer id, Double totalCost, String status, CustomerOrderCustomerDTO customer, LocalDateTime created, LocalDateTime updated) {
        CustomerOrderDTO dto = new CustomerOrderDTO();
        dto.setId(id);
        dto.setTotalCost(totalCost);
        dto.setStatus(status);
        dto.setCustomer(customer);
        dto.set_created(created);
        dto.set_updated(updated);
        return dto;
    }

    private JpaCustomerOrderEntity createJpaCustomerOrderEntity(Integer id, Double totalCost, Integer statusId, Integer customerId, LocalDateTime created, LocalDateTime updated, String customerName) {
        return new JpaCustomerOrderEntity(id, totalCost, statusId, customerId, created, updated, customerName);
    }
}
